...
function buildReceiptMarkup(order: OrderRow, items: Array<OrderItemRow & { optionSummary: string }>) {
  ...
  const itemRowsHtml = items
    .map((item) => {
      const optionLine = item.optionSummary ? `<div style=\"margin-top:4px;color:#6b7280;font-size:13px;\">${escapeHtml(item.optionSummary)}</div>` : "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (order.status !== "delivered") {
      return new Response(JSON.stringify({ error: "Receipts can only be sent for delivered orders" }), {
        status: 409,
        headers: corsHeaders,
      });
    }
    ...
    let optionsByItemId = new Map<string, string[]>();
